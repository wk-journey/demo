...
public class RedisConfiguration {
    ...
    @Bean("anotherRedisProperties")
    public AnotherRedisProperties anotherRedisProperties() {
        return new AnotherRedisProperties();
    }

    @Primary
    @Bean(name = "redisService")
    @ConditionalOnMissingBean(name = {"redisService"})
    public RedisService redisService(DefaultRedisProperties defaultRedisProperties) {
        return new RedisServiceImpl(this.defaultRedisTemplateNoTransaction(defaultRedisProperties));
    }

    @Bean(name = "anotherRedisServie")
    @ConditionalOnMissingBean(name = {"anotherRedisService", "redisService"})
    public RedisService anotherRedisServie(AnotherRedisProperties anotherRedisProperties) {
        return new AnotherRedisServiceImpl(this.anotherRedisTemplateNoTransaction(anotherRedisProperties));
    }
    ...
    @Bean(name = "anotherRedisTemplate")
    @ConditionalOnMissingBean(name = {"redisTemplate"})
    public StringRedisTemplate anotherRedisTemplateNoTransaction(AnotherRedisProperties anotherRedisProperties) {
        StringRedisTemplate template = new StringRedisTemplate(
                this.persistRedisConnectFactory(anotherRedisProperties)
        );
        this.setRedisTemplateProperties(template);
        return template;
    }
